...
#define DEFAULT_NWKSKEY \
  { 0xBA, 0xBE, 0xCA, 0xFE, 0x12, 0x34, 0x56, 0x78, 0xBA, 0xBE, 0xCA, 0xFE, 0x12, 0x34, 0x56, 0x78 }
// CAFEBABE12345678CAFEBABE12345678
#define DEFAULT_APPSKEY \
  { 0xCA, 0xFE, 0xBA, 0xBE, 0x12, 0x34, 0x56, 0x78, 0xCA, 0xFE, 0xBA, 0xBE, 0x12, 0x34, 0x56, 0x78 }
...
static lorawan_endpoint_t *lgw_sx130x_endpoint = NULL;

const lorawan_endpoint_t *endpoints_set_myself(const uint64_t deveui) {
  for (uint8_t e = 0; e < ARRAY_SIZE(lorawan_endpoints); e++) {
    if (deveui == lorawan_endpoints[e].deveui) {
      lgw_sx130x_endpoint = lorawan_endpoints + e;
      return lgw_sx130x_endpoint;
    }
  }
  return NULL;
}

const lorawan_endpoint_t *endpoints_get_myself(void) {
  return lgw_sx130x_endpoint;
}

const lorawan_endpoint_t *endpoints_get_endpoint(const uint32_t devaddr) {
  for (uint8_t e = 0; e < ARRAY_SIZE(lorawan_endpoints); e++) {
    if (devaddr == lorawan_endpoints[e].devaddr) {
      return lorawan_endpoints + e;
    }
  }
  return NULL;
}
...
void endpoints_print_all(void) {
  for (uint8_t e = 0; e < ARRAY_SIZE(lorawan_endpoints); e++) {
    lorawan_endpoint_t *le = lorawan_endpoints + e;
        Serial.print(F("0x"));
        Serial.print(le->deveui, HEX);
        Serial.print(F("  0x"));
        Serial.print(le->devaddr, HEX);
        Serial.print(F("  "));
        Serial.print(F(le->label));
        Serial.println();
    }
}
