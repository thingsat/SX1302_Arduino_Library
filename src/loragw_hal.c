...
#include "arb_fw.h"           /* text_arb_sx1302_13_Nov_3 */
#include "agc_fw_sx1250.h"    /* text_agc_sx1250_05_Juillet_2019_3 */
#include "agc_fw_sx1257.h"    /* text_agc_sx1257_19_Nov_1 */
...
int lgw_start() {
    ...
    err = lgw_connect(CONTEXT_BOARD.cs);
    ...
    if (CONTEXT_SX1261.enable == true) {
        err = sx1261_connect(CONTEXT_SX1261.cs);
        if (err != LGW_REG_SUCCESS) {
...
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
    ...
    float current_temperature = 10.0, rssi_temperature_offset = 0.0;
